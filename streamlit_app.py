...
def display_settings():
    ...
    with st.container():
        ...
        enable_ai_summary = st.checkbox(
            "Enable AI-Powered Analysis Summaries",
            value=True,
            help="Use AI APIs to generate detailed, contextual analysis summaries"
        )
        ...
        if enable_ai_summary:
            ai_provider = st.selectbox(
                ...
                ["openrouter", "groq"],
                index=0,
                help="Choose between OpenRouter (supports multiple models) or Groq (fast inference)"
            )
            
            if ai_provider == "openrouter":
                st.info("**OpenRouter**: Supports multiple AI models including Claude, GPT, and others. Requires API key.")
                ...
                if openrouter_key:
                    ...
                    st.warning("⚠️ OpenRouter API key required for AI summaries")
            
            elif ai_provider == "groq":
                st.info("**Groq**: Ultra-fast inference with Llama models. Requires API key.")
                ...
                if groq_key:
                    ...
                    st.warning("⚠️ Groq API key required for AI summaries")
            ...
            summary_length = st.slider(
                ...
                value=500,
                step=50,
                help="Maximum number of words in the AI summary"
            )
            
            summary_creativity = st.slider(
                "Summary Creativity",
                min_value=0.0,
                max_value=1.0,
                value=0.7,
        ...
        if st.button("💾 Save AI Settings", type="primary"):
            st.session_state.enable_ai_summary = enable_ai_summary
            if enable_ai_summary:
                st.session_state.ai_provider = ai_provider
                st.session_state.summary_length = summary_length
                st.session_state.summary_creativity = summary_creativity
    ...
    with st.container():
        ...
        accent_color = st.color_picker(
            "Accent Color",
            value="#6366f1",
            help="Choose a color that matches your brand or preference"
        )
                ...
                try:
                    ...
                    for scan in st.session_state.scan_history:
                        try:
                            ...
                            if isinstance(timestamp, str):
                                date_str = timestamp[:19] if len(timestamp) >= 19 else timestamp
                            else:
                                date_str = timestamp.strftime('%Y-%m-%d %H:%M')
                            
                            df_data.append({
                                'Date': date_str,
                                'Filename': scan.get('filename', 'N/A'),
                                'Match Score': result.get('match_score', 0),
                                'Fit': result.get('fit', 'N/A')
                            })
                        except Exception as e:
                            logger.error(f"Error processing scan for export: {e}")
                            continue
                    ...
                    if df_data:
                        ...
                        st.download_button(
                            label="💾 Download CSV",
                            data=csv,
                            file_name="resume_scan_history.csv",
                            mime="text/csv"
                        )
                    else:
                        st.warning("No valid data available to export.")
                except Exception as e:
                    st.error(f"Error creating export: {e}")
            else:
                st.warning("No scan history available to export.")
    ...
    with st.container():
        ...
        st.markdown("""
        **ResumeScan AI** helps you optimize your resume for job applications by 
        analyzing how well it matches job descriptions.
